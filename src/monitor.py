...
def main() -> None:
    ...
    for day_idx in range(7):
        ...
        print(f"■ {days[day_idx]} | 적용 Shift 편향: {current_shift:>5.1f} | 계산된 실제 Balanced Accuracy: {current_acc:.4f}")
    
    # 4. 최종 오염 상태(Day 7) 기준 통계적 KS 검정 리포트 출력
    print("\n=== [최종 단계] Day 7 기준 비모수형 이표본 KS 검정 진단 ===")
    ks_stat, p_value = ks_2samp(X_train[TARGET_FEATURE], X_test_operational[TARGET_FEATURE])
    ...
    axes[0].hist(X_test_operational[TARGET_FEATURE], bins=15, alpha=0.5, label="Production (Day 7)", density=True, color="red")
    ...
    axes[1].plot(days, actual_accuracy_trend, marker='o', color='purple', linewidth=2, label="Calculated Balanced Accuracy")
