    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        ...
        for i, col in enumerate(X_df.columns):
            X_df[col] = X_df[col].clip(
                lower=self.lower_bounds_[i], upper=self.upper_bounds_[i]
            )
...
def load_n_clean_data(data_path: Path) -> pd.DataFrame:
    ...
    if not data_path.exists():
        raise FileNotFoundError(
            f"정밀 진단 에러: 마스터 데이터 소스가 지정된 경로에 없습니다 -> {data_path}"
        )

    raw_df = pd.read_csv(
        data_path, header=None, names=HEART_DISEASE_SCHEMA, na_values="?"
    )
